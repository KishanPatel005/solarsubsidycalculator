...
function toPortalHref(hostOrUrl: string) {
...
export default function StateSubsidyPage({ params }: PageProps) {
  ...
  const solarFacts = stateData[content.stateSlug];
    ...
    <div className="space-y-10">
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Badge variant="secondary">Central subsidy confirmed</Badge>
          <Badge variant="outline">State bonus — verify on official portal</Badge>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Card className="p-4">
            ...
            <div className="mt-1 text-xs text-muted-foreground">Verify on official portal</div>
          </Card>
          <Card className="p-4">
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <h2 className="text-xl font-semibold">State Solar Facts</h2>
          {solarFacts?.sunGrade ? (
            <Badge className="bg-solar-600 text-white hover:bg-solar-700">
              {solarFacts.sunHours.toFixed(1)} hrs/day • Grade {solarFacts.sunGrade}
            </Badge>
          ) : (
            <Badge variant="secondary">Updating local data</Badge>
          )}
        </div>
        ...
        {solarFacts ? (
          <div className="grid gap-4 lg:grid-cols-3">
            ...
            <Card className="p-4">
              <p className="text-sm font-semibold text-foreground">Approvals, policy & tariff</p>
              <ul className="mt-3 space-y-2 text-sm text-muted-foreground">
                <li className="flex items-start gap-2">
                  <CheckCircle2 className="mt-0.5 h-4 w-4 text-emerald-600" />
                  <span>
                    Average approval time: <span className="font-medium text-foreground">{solarFacts.approvalDays}</span>
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  <CheckCircle2 className="mt-0.5 h-4 w-4 text-emerald-600" />
                  <span>
                    Policy: <span className="font-medium text-foreground">{solarFacts.policy}</span>
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  <CheckCircle2 className="mt-0.5 h-4 w-4 text-emerald-600" />
                  <span>
                    Avg tariff: <span className="font-medium text-foreground">{solarFacts.avgTariff}</span>
                  </span>
                </li>
                {typeof solarFacts.stateBonus === "number" ? (
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="mt-0.5 h-4 w-4 text-emerald-600" />
                    <span className="font-medium text-emerald-700">
                      State bonus: {formatINR(solarFacts.stateBonus)}
                      {solarFacts.stateBonusNote ? ` — ${solarFacts.stateBonusNote}` : ""}
                    </span>
                  </li>
                ) : null}
              </ul>
            ...
            <Card className="p-4">
              <p className="text-sm font-semibold text-foreground">Best districts & portal</p>
              <ul className="mt-3 space-y-2 text-sm text-muted-foreground">
                {solarFacts.bestDistricts.map((d) => (
                  <li key={d} className="flex items-start gap-2">
                    <CheckCircle2 className="mt-0.5 h-4 w-4 text-emerald-600" />
                    <span>{d}</span>
                  </li>
        ...
        <p className="text-xs text-muted-foreground">
          Note: Sun hours and approval timelines are planning averages. Always confirm the latest process, net metering and bonus benefits on the
          official portal and with your DISCOM.
        </p>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <Badge variant="outline">Verify bonus on official portal</Badge>
        </div>
